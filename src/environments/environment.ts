...
class Environment implements IEnvironment {
    ...
    constructor(NODE_ENV?: string) {
      const env: string= NODE_ENV || configDotenv().parsed.NODE_ENV || Environments.DEV;
      const port: string | undefined | number = configDotenv().parsed.PORT || 3146;
      this.setEnvironment(env);
      this.port = Number(port);
      this.applyEncryption = JSON.parse(configDotenv().parsed.APPLY_ENCRYPTION);
      this.secretKey =  configDotenv().parsed.SECRET_KEY;
    }

    public getCurrentEnvironment(): string {
      let environment: string = configDotenv().parsed.NODE_ENV || Environments.DEV;
